...
func (repo *repository) Store(table string, data interface{}) error {
	...
	switch table {
		case "customers":
			...
			stmt, err := db.Prepare(QUERY_STORE_CUSTOMERS); if err != nil {
				return err
			}

			defer stmt.Close()

			_, err = stmt.Exec(customerStored.ID, customerStored.LastName, customerStored.FirstName, customerStored.Condition); if err != nil {
				return err
			}
		...
		case "products":
			...
			stmt, err := db.Prepare(QUERY_STORE_PRODUCTS); if err != nil {
				return err
			}

			defer stmt.Close()

			_, err = stmt.Exec(productsStored.ID, productsStored.Description, productsStored.Price); if err != nil {
				return err
			}
		...
		case "invoices":
			...
			stmt, err := db.Prepare(QUERY_STORE_INVOICES); if err != nil {
				return err
			}

			defer stmt.Close()

			_, err = stmt.Exec(invoicesStored.ID, invoicesStored.DateTime, invoicesStored.IDCustomer); if err != nil {
				return err
			}
		...
		case "sales":
			...
			stmt, err := db.Prepare(QUERY_STORE_SALES); if err != nil {
				return err
			}

			defer stmt.Close()

			_, err = stmt.Exec(salesStored.ID, salesStored.IDInvoice, salesStored.IDProduct, salesStored.Quantity); if err != nil {
				return err
			}
...
func (repo *repository) Enunciado1() ([]models.Enunciado1, error) {
	...
	rows, err := db.Query(QUERY_ENUNCIADO_1); if err != nil {
		return nil, err
	}
...
func (repo *repository) Enunciado2() ([]models.Enunciado2, error) {
	...
	rows, err := db.Query(QUERY_ENUNCIADO_2); if err != nil {
		return nil, err
	}
...
func (repo *repository) Enunciado3() ([]models.Enunciado3, error) {
	...
	rows, err := db.Query(QUERY_ENUNCIADO_3); if err != nil {
		return nil, err
	}

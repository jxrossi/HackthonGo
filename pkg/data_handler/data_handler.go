...
func InsertData(table string, service internal.Service) string {
	...
	switch table {
	case "customers":
		//Check if given table already has data.
		if !(service.ExistsCustomers()) {
			...
			for _, line := range lines {
				...
				if err := service.Store(table, custom); err != nil {
					return fmt.Sprintf("Error: %s\n", err)
				}
			}
	...
	case "products":
		if !(service.ExistsProducts()) {
			for _, line := range lines {
				...
				if err := service.Store(table, prods); err != nil {
					return fmt.Sprintf("Error: %s\n", err)
				}
			}
	...
	case "invoices":
		if !(service.ExistsInvoices()) {
			for _, line := range lines {
				...
				if err := service.Store(table, invs); err != nil {
					return fmt.Sprintf("Error: %s\n", err)
				}
			}
	...
	case "sales":
		if !(service.ExistsSales()) {
			for _, line := range lines {
				...
				if err := service.Store(table, sales); err != nil {
					return fmt.Sprintf("Error: %s\n", err)
				}
			}
	...
	if !(service.ExistsTotalsInvoices()) {
		if service.ExistsSales() {
			for _, line := range lines {
				...
				err := service.StoreTotalInvoices(id)
				if err != nil {
					return fmt.Sprintf("Error: %s\n", err)
				}
			}
